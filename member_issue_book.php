...
<body>
    ...
    <div class="main-container">
        ...
        <div class="issued-books">
            <h1  style="color:#45076a; text-align:center;">Books Issued to You</h1>
            <table>
                <thead>
                    <tr>
                        <th>Book Image</th>
                ...
                <tbody>
                <?php
                // Fetch books issued to the logged-in member where status is 'issued'
                $fetch_issued_books_query = "SELECT book.book_id, book.book_title, book.book_image, issue_book.due_date 
                                             FROM book 
                                             JOIN issue_book ON book.book_id = issue_book.book_id 
                                             WHERE issue_book.member_id = '$logged_in_member_id' AND issue_book.status = 'issued'";
                $issued_books_result = $conn->query($fetch_issued_books_query);
                if ($issued_books_result->num_rows > 0) {
                    while ($row = $issued_books_result->fetch_assoc()) {
                        echo "<tr>";
