...
<a href="index.php" style="text-decoration:none; color:black;">
        ...
        <div class="main">
            <div>
                <h1 style="text-align:center; margin-bottom:5px; color:#45076a;">Manage Books</h1>
                <div style="text-align:right">
                <button type="button" class="btn add-btn" title="Add">
                                    <a href="add_book.php" class="btn-link">
...
if (isset($_POST['search'])) {
    ...
    $sql = "SELECT * FROM book 
            WHERE book_id LIKE '%$search%' OR book_title LIKE '%$search%' 
            OR author_name LIKE '%$search%' OR book_isbn LIKE '%$search%' 
            OR domain_name LIKE '%$search%'  OR rack_no LIKE '%$search%'
            LIMIT $start_from, $results_per_page";
} else {
    // Default query to display books with pagination
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td><img src='" . $row["book_image"] . "' alt='Book Image' width='100'></td>";
        ...
        if (!empty($row["book_pdf"])) {
            echo "<td><a href='" . $row["book_pdf"] . "' target='_blank' style='color:green; text-decoration:none;'>✔️ </a></td>";
        } else {
        ...
        echo '<td> 
                <button type="button" class="btn edit-btn" title="Edit">
                    <a href="update_book.php?id=' . $row['book_id'] . '" class="btn-link">
...
<div class="pagination">
    <?php
    if ($current_page > 1) {
        echo "<a href='book.php?page=" . ($current_page - 1) . "'>&laquo; Previous</a>";
    }

    for ($i = 1; $i <= $total_pages; $i++) {
        if ($i == $current_page) {
            echo "<a class='active' href='book.php?page=" . $i . "'>" . $i . "</a>";
        } else {
